...
def run_bedtools_makewindows(
    in_bed_file: str,
    out_file: str,
    window_size=1e5,
):
    print("run bedtools makewindows")
    subprocess.run(
        [
            "bedtools",
            "makewindows",
...
def run_bedtools_coverage(
    ...
    if load_df:
        out_df = pd.read_table(out_file, sep="\t", header=None, usecols=range(ncols))
        out_df.columns = names
        return out_df
    return None
